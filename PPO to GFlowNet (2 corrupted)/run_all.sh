#!/usr/bin/env bash
# Run both corruption studies for Study A (PPO corrupted -> GFlowNet original):
# 3 arms x 30 seeds each.
#
...
for corr in "${CORRUPTIONS[@]}"; do
  ...
  for ((s = 0; s < NSHARDS; s++)); do
    ...
    (
      python3 run_study.py --study A --corruption "$corr" --seeds "${shard[@]}" \
...
for corr in "${CORRUPTIONS[@]}"; do
  ...
  python3 run_study.py --study A --corruption "$corr" --seeds "${SEEDS[@]}" \
