...
class MazeGeodesic:
    ...
    def __init__(self, maze_map, resolution: float = DEFAULT_RESOLUTION,
        ...
        # Reduced to a pure wall/free mask: a map may carry 'r'/'g'/'c' goal
        # and reset markers (LARGE_MULTIGOAL does), and those cells are free
        # space.  Treating a marker as a wall would inflate L* and silently
        # flatter every agent's path efficiency on that layout.
        self.maze_map = wall_mask(maze_map)
...
def get_geodesic(maze_map, resolution: float = DEFAULT_RESOLUTION,
    ...
    key = tuple(tuple(row) for row in wall_mask(maze_map))
