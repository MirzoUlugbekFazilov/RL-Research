...
class PPOAgent:
    ...
    def from_ppo(cls, checkpoint, variant: str = "original", *,
                 ...
                 surface: str = "policy_only") -> "PPOAgent":
        """Start PPO from a **PPO** checkpoint -- Study C's fine-tuning contract.

        ``surface='policy_only'`` copies the six actor tensors into an otherwise
        fresh agent, so the critic stays randomly initialised and Adam starts
        with zero moments.  That is not a limitation of the implementation: it
        reproduces exactly what Study B's GFlowNet hands to PPO, which is what
        makes "GFlowNet pretraining vs PPO pretraining" a one-variable contrast.

        ``surface='full'`` delegates to :meth:`finetune_from`, so actor, critic
        and optimiser moments all carry over -- ordinary PPO fine-tuning, run as
        the secondary arm that prices the fresh critic.

        Either way the resulting agent carries ``transfer_report``, which
        ``run_phase2`` writes into the run's ``transfer.json``.
        """
        from algorithms.transfer import (transfer_ppo_policy_to_ppo,
                                         verify_ppo_full_load)

        if surface not in ("policy_only", "full"):
            raise ValueError(f"unknown transfer surface {surface!r}")

        if surface == "full":
            agent = cls.finetune_from(
                checkpoint, variant, corruption=corruption, discrete=True,
                seed=seed, n_envs=n_envs, action_repeat=action_repeat,
                time_feature=time_feature,
            )
            agent.transfer_report = verify_ppo_full_load(
                agent.model.policy, checkpoint,
                obs_space=agent.model.observation_space, seed=seed,
            )
            return agent
        ...
        agent.transfer_report = transfer_ppo_policy_to_ppo(
            agent.model.policy, checkpoint,
            obs_space=agent.model.observation_space, seed=seed,
        )
        agent._loaded_from = str(checkpoint)
