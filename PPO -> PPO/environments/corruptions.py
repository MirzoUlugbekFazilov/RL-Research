...
from .layouts import LARGE_MULTIGOAL, UMAZE
...
def large_multigoal_layout(m):
    """Replace the maze wholesale with :data:`~environments.layouts.LARGE_MULTIGOAL`.

    Unlike every other layout transform in this module this one **ignores its
    argument**: it does not rearrange UMaze, it swaps in a different, larger
    maze that carries several goal cells.  That is deliberate, and it is why
    :data:`STRUCTURAL_CORRUPTIONS` lists it separately -- see the note on
    difficulty preservation there.
    """
    return [list(r) for r in LARGE_MULTIGOAL]


@dataclass(frozen=True)
class Corruption:
    name: str
    #: ``"layout"`` | ``"actuation"`` | ``"actuation_stochastic"`` | ``"none"``
    mechanism: str
    layout_fn: Callable = identity_layout
    action_matrix: np.ndarray | None = None
    #: ``(low, high)`` for ``mechanism == "actuation_stochastic"``: the support
    #: of the per-episode actuator gain ``g``.
    gain_range: tuple[float, float] | None = None
    rationale: str = ""
    #: False for corruptions that are *not* isomorphic to the original MDP, so
    #: a transfer gap cannot be cleanly separated from a difficulty change.
    difficulty_preserving: bool = True
...
CORRUPTION_FAMILY: dict[str, Corruption] = {
    ...
    # -- structural mechanism (this study) --------------------------------
    "large_multigoal": Corruption(
        "large_multigoal", "layout", layout_fn=large_multigoal_layout,
        difficulty_preserving=False,
        rationale="A larger maze carrying several goals: gymnasium-robotics' "
                  "LARGE_MAZE_DIVERSE_G (12x9, 7 goal cells, 1 reset cell) "
                  "replaces UMaze (5x5, 9 free cells, start and goal drawn "
                  "from anywhere). NOT difficulty-preserving -- see "
                  "STRUCTURAL_CORRUPTIONS.",
    ),
    # -- stochastic actuation mechanism (this study) ----------------------
    "action_gain_jitter": Corruption(
        "action_gain_jitter", "actuation_stochastic", gain_range=(0.9, 1.1),
        rationale="Actuator gain u -> g*u with g ~ U(0.9, 1.1) drawn once per "
                  "episode and held fixed. Mild domain randomisation whose "
                  "support contains the original (g = 1), so it is a "
                  "*generalisation* corruption rather than a conflicting one.",
    ),
}

#: Corruptions whose corrupted MDP is **not** isomorphic to the original, so a
#: transfer gap cannot be attributed to transfer alone.
#:
#: ``large_multigoal`` changes the maze, its size, the free-cell count, the
#: start distribution (one reset cell instead of nine) and the goal
#: distribution (seven designated cells instead of any free cell).  It is
#: therefore a genuinely *different and harder* task, not a relabelling of the
#: original one, and the pretrain arm's native score is not comparable to the
#: original environment's.  This is disclosed rather than argued away: for this
#: corruption the fine-tune-vs-scratch contrast remains perfectly valid (both
#: arms still run the same algorithm on the same original environment with the
#: same budget), but the *zero-shot* number mixes "the policy is wrong here"
#: with "the policy never saw a task like this".
STRUCTURAL_CORRUPTIONS = ("large_multigoal",)

#: Corruptions that are *stochastic*: the corrupted environment is a
#: distribution over MDPs rather than a single one.  ``action_gain_jitter``'s
#: support **contains the original** (g = 1 is interior to U(0.9, 1.1)), which
#: makes it the one corruption in this codebase for which positive transfer is
#: the a-priori expectation -- it is domain randomisation, and the standard
#: claim about domain randomisation is that it *helps*.
STOCHASTIC_CORRUPTIONS = ("action_gain_jitter",)
