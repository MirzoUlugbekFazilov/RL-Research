...
#: Set by :func:`main` (or :func:`select_corruption`) to
#: ``runs/<corruption>/{results,checkpoints}``.  One corruption per directory,
#: so two corruptions can never write into each other's results.
RESULTS = RUNS / "unset" / "results"
CHECKPOINTS = RUNS / "unset" / "checkpoints"


def select_corruption(corruption: str) -> None:
    """Point the whole run at one corruption.

    ``hyperparameters.py`` is kept **byte-identical** to the original study's
    (SHA-256 ``d01ab117...``), which is what makes "same hyperparameters, only
    the corruption differs" a checkable claim rather than a promise.  The
    corruption is therefore *not* edited into that file; it is overridden here,
    at run time, and recorded in every ``config.json``.

    Every module reads the corruption through ``HP.CORRUPTION`` or through an
    explicit argument threaded from it, so this single assignment is sufficient
    -- ``grep -rn CORRUPTION`` shows no other source.
    """
    global RESULTS, CHECKPOINTS
    from environments.corruptions import get

    get(corruption)  # raises on an unknown name before anything expensive runs
    HP.CORRUPTION = corruption
    RESULTS = RUNS / corruption / "results"
    CHECKPOINTS = RUNS / corruption / "checkpoints"
...
STUDIES = {
    ...
    # -- Study C: the same-algorithm control the cross studies were missing --
    #
    # A and B each ask "does pretraining on the corrupted environment help?"
    # against a from-scratch control.  Neither asks "was pretraining with the
    # *other* algorithm worth it?", because neither ever runs the matched
    # same-algorithm pretraining.  C does: PPO on corrupted -> PPO on original,
    # against the identical from-scratch control B used.
    #
    # C and C_full share the pretrain and scratch arms by name, so running both
    # trains each of those once and the second study finds them cached.  Only
    # the fine-tuning arm differs, which is the only thing that does differ.
    "C": dict(
        name="C",
        directory="PPO + PPO",
        title="PPO on corrupted -> PPO fine-tuned on original (actor copy only)",
        pretrain_algo="ppo",
        finetune_algo="ppo",
    ...
    "C_full": dict(
        name="C_full",
        directory="PPO + PPO",
        title="PPO on corrupted -> PPO fine-tuned on original (full model)",
        pretrain_algo="ppo",
        finetune_algo="ppo",
        transfer_surface="full",
        arms=dict(
            pretrain="ppo_pretrain_corrupted",
            finetune="ppo_finetune_full_original",
            scratch="ppo_scratch_original",
        ),
    ),
}
...
def transfer_agent(study: dict, checkpoint: Path, seed: int):
    """The fine-tuning agent, initialised from phase 1's checkpoint.

    Dispatches on whether the study crosses algorithms (A, B) or keeps the same
    one (C).  Every branch ends in an exact parameter copy verified numerically
    inside :mod:`algorithms.transfer`, so in all three studies phase 2 provably
    begins at the pretrained policy and spends its whole budget on the original
    environment.
    """
    if study["pretrain_algo"] == study["finetune_algo"]:
        return same_algo_agent(study, checkpoint, seed)
    return crossover_agent(study["finetune_algo"], checkpoint, seed)


def same_algo_agent(study: dict, checkpoint: Path, seed: int):
    """Study C: PPO continues from PPO, on the surface the study declares.

    ``policy_only`` restricts the copy to the actor, matching exactly what a
    GFlowNet can hand to PPO in Study B; ``full`` carries the critic and the
    optimiser moments too.  See :mod:`algorithms.transfer` for why both are run.
    """
    if study["finetune_algo"] != "ppo":
        raise NotImplementedError(
            "same-algorithm transfer is implemented for PPO -> PPO only; "
            "a GFlowNet -> GFlowNet study would need its logZ head handled"
        )
    return PPOAgent.from_ppo(
        checkpoint, "original",
        corruption=HP.CORRUPTION, seed=seed, n_envs=HP.PPO_N_ENVS,
        action_repeat=HP.ACTION_REPEAT, policy_hidden=(HP.HIDDEN, HP.HIDDEN),
        time_feature=HP.TIME_FEATURE,
        surface=study.get("transfer_surface", "policy_only"),
    )
...
def run_phase2(study: dict, seed: int, kind: str, pretrained: Path | None,
    ...
    t0 = time.time()
    if kind == "finetune":
        agent = transfer_agent(study, pretrained, seed)
    ...
    write_json(out / "config.json", dict(
        ...
        transfer_surface=(study.get("transfer_surface") if kind == "finetune"
                          else None),
        parameter_fingerprint=agent.parameter_fingerprint(),
        wall_clock_seconds=round(time.time() - t0, 1),
        hyperparameters=HP.resolved(), versions=package_versions(),
...
def main() -> None:
    ...
    ap.add_argument("--corruption", default=HP.CORRUPTION,
                    help="corruption to pretrain on (see environments/corruptions.py)")
    ...
    ap.add_argument("--no-study-json", action="store_true",
                    help="skip writing results/study.json (for sharded workers)")
    args = ap.parse_args()

    select_corruption(args.corruption)
    ...
    if not args.no_study_json:
        record = dict(
            study, corruption=args.corruption,
            hyperparameters=dict(HP.resolved(), corruption=args.corruption),
            versions=package_versions(),
            seeds_run=args.seeds, wall_clock_seconds=round(time.time() - t0, 1),
        )
        # Two studies (C and C_full) share one results tree, so the record is
        # keyed by study name as well as written to the unsuffixed default.
        # ``analyze.py --study`` reads the suffixed file; without this the
        # second study to finish would silently relabel the first one's runs.
        write_json(RESULTS / f"study_{study['name']}.json", record)
        write_json(RESULTS / "study.json", record)
