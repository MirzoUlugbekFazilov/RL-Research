...
RESULTS = ROOT / "runs" / "unset" / "results"

#: One-line description of each corruption, and the interpretation caveat that
#: goes with it. Keyed by corruption name; used verbatim in the report so the
#: caveat cannot drift away from the numbers it applies to.
CORRUPTION_PROSE = {
    "negate_both": (
    ...
    "large_multigoal": (
        "a larger maze carrying several goals (`LARGE_MAZE_DIVERSE_G`, 12x9, "
        "7 goal cells) replaces UMaze (5x5, 9 free cells)",
    ...
    "action_gain_jitter": (
        "per-episode actuator gain `u -> g*u`, `g ~ U(0.9, 1.1)`, drawn at reset",
...
def corruption_prose(name: str) -> tuple[str, str]:
    return CORRUPTION_PROSE.get(
        name, (f"`{name}`", "No interpretation note is registered for this corruption.")
    )
...
def main() -> None:
    ...
    ap.add_argument("--corruption", required=True,
                    help="which runs/<corruption>/ tree to aggregate")
    ap.add_argument("--study", default=None,
                    help="study name when one results tree holds several "
                         "(C and C_full share pretrain and scratch); reads "
                         "results/study_<name>.json instead of study.json")
    ap.add_argument("--out", default=None,
                    help="basename for the report (default METRICS_<corruption>)")
    args = ap.parse_args()

    RESULTS = ROOT / "runs" / args.corruption / "results"
    study_file = RESULTS / (f"study_{args.study}.json" if args.study
                            else "study.json")
    if not study_file.exists():
        raise SystemExit(f"no {study_file} -- run run_study.py first")
    meta = json.loads(study_file.read_text())
    study = STUDIES[meta["name"]]
    arms = study["arms"]

    # The report is written from HP, so point HP at the corruption these runs
    # actually used rather than at hyperparameters.py's default.
    HP.CORRUPTION = meta.get("corruption", HP.CORRUPTION)
    corr_desc, corr_note = corruption_prose(HP.CORRUPTION)
    # With several studies in one tree the corruption alone no longer names a
    # report, so the study name joins it unless the caller overrides.
    out_base = args.out or (f"METRICS_{HP.CORRUPTION}_{args.study}" if args.study
                            else f"METRICS_{HP.CORRUPTION}")
    ...
    (ROOT / f"{out_base}.json").write_text(
    ...
    A(f"- Corruption: `{HP.CORRUPTION}` ({corr_desc})")
    ...
    A("")
    same_algo = study["pretrain_algo"] == study["finetune_algo"]
    surface = study.get("transfer_surface")
    A(f"The {'same' if same_algo else 'cross'}-algorithm initialisation is an "
      "**exact parameter copy**, not a")
    ...
    if not same_algo:
        ...
        A("comparable.")
    elif surface == "policy_only":
        A("What is *not* transferred, and deliberately so: the critic and the optimiser")
        A("moments. PPO could hand both over — see the `_full` report, which does — but a")
        A("GFlowNet cannot, so this arm gives up exactly what study B's arm was forced to,")
        A("and the two become a one-variable contrast in *which algorithm pretrained*.")
        A("The critic is randomly initialised in this arm and in the scratch control alike,")
        A("so it is never a difference between them.")
    else:
        A("Everything transfers here: actor, critic and Adam moments, by `PPO.load`. This")
        A("is ordinary PPO fine-tuning, and it is the arm the cross-algorithm studies could")
        A("not run. Its gap over the actor-only arm is the price of the value function a")
        A("GFlowNet has no way to provide. The scratch control still starts from a random")
        A("critic, so this arm — unlike every other in the three studies — holds an")
        A("advantage over its control beyond initialisation, and its contrast should be")
        A("read with that in mind.")
    A("")

    (ROOT / f"{out_base}.md").write_text("\n".join(L))
    print("\n".join(L))
    print(f"\nwrote {ROOT / (out_base + '.json')} and {ROOT / (out_base + '.md')}")
