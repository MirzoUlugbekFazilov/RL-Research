# GFlowNet → PPO transfer under two new corruptions

Train **only a GFlowNet** on a corrupted `PointMaze_UMaze-v3`, then continue
from that policy with **only PPO** on the uncorrupted environment, and measure
whether the corrupted-environment pretraining helps.

This is the design of `../RL+PPO` (Study B), re-run unchanged against two
...
./run_all.sh 6                                    # 3 arms x 30 seeds x 2 corruptions, ~25 min
...
original environment. That rule maximises how wrong the pretrained policy is at
the start of fine-tuning, so its strong negative-transfer result is close to
being built into the choice of corruption. Neither corruption here is selected
that way, and they sit on opposite sides of the interesting question:
...
unaffected by it**: `ppo_finetune_original` and `ppo_scratch_original` both run
PPO on the same original environment with the same budget and the same
...
`environments/corruptions.py::STRUCTURAL_CORRUPTIONS` carries the same warning
next to the definition, and `analyze.py` prints it into the report next to the
numbers it applies to, so the caveat cannot drift away from them.
...
`hyperparameters.py` is **byte-identical** to `../RL+PPO/hyperparameters.py`
(SHA-256 `d01ab117…`) — same budgets, same 30 seeds, same network, same PPO and
GFlowNet settings, same evaluation protocol. The corruption is *not* edited into
that file; it is a run-time argument (`run_study.py --corruption`), recorded in
every run's `config.json`, so "same hyperparameters, only the corruption
differs" stays a checkable claim:
...
```
hyperparameters.py         byte-identical to ../RL+PPO's; SHA in every config.json
run_study.py               --corruption <name>; runs the 3 arms x 30 seeds
run_all.sh                 both corruptions, sharded over worker processes
analyze.py                 --corruption <name> -> METRICS_<name>.{md,json}
combine_metrics.py         the two reports + ../RL+PPO's -> METRICS.md
