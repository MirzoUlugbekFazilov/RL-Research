...
def make_env(
    ...
    *,
    ...
    if active and corr.mechanism == "actuation_stochastic":
        # Same slot as the deterministic actuation faults -- inside the
        # discretiser -- so the bang-bang table is scaled exactly as a
        # continuous command would be. The wrapper reaches into MuJoCo's
        # `actuator_gear` rather than multiplying the action array, because the
        # env clips actions to [-1, 1] and every bang-bang action sits on that
        # boundary; see RandomActionGain's docstring for the measurement.
        env = RandomActionGain(env, *corr.gain_range, seed=seed)
    if discrete:
