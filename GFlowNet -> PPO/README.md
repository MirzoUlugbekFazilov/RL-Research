# Study B — GFlowNet on corrupted PointMaze → PPO fine-tuned on the original

Train **only GFlowNet** on a corrupted `PointMaze_UMaze-v3`, then continue from
that policy with **only PPO** on the uncorrupted environment, and measure
whether the corrupted-environment pretraining helps.

Its mirror image lives in `../RL+gflowne` (PPO → GFlowNet). The two directories
run **byte-identical code with byte-identical hyperparameters**; only the roles
of the two algorithms are swapped.

```bash
cd "$(dirname "$0")"
python3 run_study.py --study B     # ~6 min: trains all 3 arms x 5 seeds
...
|---|---|---|---|---|
| `gfn_pretrain_corrupted` | GFlowNet | **corrupted** | 300k steps | random |
| `ppo_finetune_original` | PPO | **original** | 300k steps | ← GFlowNet's policy, exact weight copy |
| `ppo_scratch_original` | PPO | **original** | 300k steps | random |

`ppo_scratch_original` is the control, and it is what makes the study mean
anything. "The fine-tuned PPO reaches success rate *X*" is not a result on its
own — the question is whether starting from the GFlowNet's corrupted-environment
...
The pretrained GFlowNet is additionally evaluated **zero-shot** on the original
...
`PPOAgent.from_gflownet` copies the GFlowNet's forward policy into PPO's actor
...
provably begins at the GFlowNet's own policy.

The copy is **verified numerically rather than trusted** — the GFlowNet's and
PPO's logits are compared on 512 probe observations drawn from the whole
...
**What is not transferred:** PPO's critic is randomly initialised. A GFlowNet
has no value function, and its `log Z` head is not one — `log Z(c)` estimates
the log partition function of the whole trajectory distribution for the initial
(start, goal) instance, whereas a critic estimates expected return-to-go from
the *current* state. Copying one into the other would be a category error.
Study A gives up exactly one head too (the GFlowNet's `log Z` head, since PPO
has no partition function), which is what keeps the two studies comparable.
This asymmetry can only ever *disadvantage* the cross-algorithm arm, so it is
stated with the results rather than hidden.
...
```
hyperparameters.py    single source of truth; byte-identical to ../RL+gflowne's
...
diff -r . ../RL+gflowne --exclude=results --exclude=checkpoints \
